...
function renderBlock(b: Block, ctx: Ctx) {
  ...
  switch (b.type) {
    ...
    case "order_bumps":
      // FIX DEFINITIVO: Habilitamos o bloco se b.enabled estiver true.
      // E mostramos todos os bumps, filtrando apenas se explicitamente ocultos no schema.
      const rawBumps = product.order_bumps || [];
      const orderBumpVisibility = config.orderBumpVisibility || {};
      
      const visibleBumps = rawBumps.filter(bump => orderBumpVisibility[bump.id] !== false);
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-semibold truncate">{bump.name}</span>
          ...
          <div className="mt-4 space-y-1.5 text-sm">
            ...
            {Array.isArray(ctx.includeBump) ? (
              product.order_bumps?.filter(b => (ctx.includeBump as any).includes(b.id)).map(b => (
                <Row key={b.id} label={b.name} value={BRL(b.price_cents / 100)} />
              ))
            ) : (
