...
export function normalizeConfig(raw: unknown, brand = "#E10600", productBumps?: any[]): CheckoutConfig {
  ...
  const hasOrderBumps = blocks.some(b => b.type === "order_bumps");
  if (!hasOrderBumps) {
    ...
    // Força o campo enabled: true para todos os blocos order_bumps existentes
    blocks = blocks.map(b => b.type === "order_bumps" ? { ...b, enabled: true } as Block : b);
  }
  ...
  return {
    ...
    blocks,
    orderBumpVisibility: { ...(cfg.orderBumpVisibility ?? {}) },
  };
}
