...
export type CheckoutConfig = {
  ...
  fields: FieldConfig[];
  blocks: Block[];
  /** Per-checkout visibility for Order Bumps. Keys are bump IDs. */
  orderBumpVisibility?: Record<string, boolean>;
};
...
export function normalizeConfig(raw: unknown, brand = "#E10600", productBumps?: any[]): CheckoutConfig {
  const base = defaultConfig(brand);
  if (!raw || typeof raw !== "object") return base;
  const cfg = raw as Partial<CheckoutConfig>;
  
  let blocks = Array.isArray(cfg.blocks) && cfg.blocks.length ? [...cfg.blocks] as Block[] : [...base.blocks];
  
  // GARANTIA TOTAL: O bloco order_bumps DEVE existir e estar HABILITADO.
  const bumpBlockIdx = blocks.findIndex(b => b.type === "order_bumps");
  if (bumpBlockIdx === -1) {
    const formIdx = blocks.findIndex(b => b.type === "form");
    const bumpBlock: Block = { id: uid(), type: "order_bumps", enabled: true };
    if (formIdx !== -1) {
      blocks.splice(formIdx + 1, 0, bumpBlock);
    } else {
      blocks.push(bumpBlock);
    }
  } else {
    // Força o campo enabled: true para o bloco order_bumps existente
    blocks[bumpBlockIdx] = { ...blocks[bumpBlockIdx], enabled: true } as Block;
  }

  // Garantia de visibilidade: Se houver bumps do produto, garante que estão no mapa de visibilidade como true por padrão
  const orderBumpVisibility = { ...(cfg.orderBumpVisibility ?? {}) };
  if (productBumps && Array.isArray(productBumps)) {
    productBumps.forEach(bump => {
      if (orderBumpVisibility[bump.id] === undefined) {
        orderBumpVisibility[bump.id] = true;
      }
    });
  }
